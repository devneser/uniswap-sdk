...
import { ChainId, TLOS, CurrencyAmount, Pair, Percent, Route, Token, TokenAmount, Trade, TradeType, WETH } from '../src'
...
describe('Trade', () => {
  ...
  it('can be constructed with TLOS as input', () => {
    const trade = new Trade(
      new Route([pair_weth_0], TLOS),
      CurrencyAmount.ether(JSBI.BigInt(100)),
      TradeType.EXACT_INPUT
    )
    expect(trade.inputAmount.currency).toEqual(TLOS)
    expect(trade.outputAmount.currency).toEqual(token0)
  })
  it('can be constructed with TLOS as input for exact output', () => {
    const trade = new Trade(
      new Route([pair_weth_0], TLOS, token0),
  ...
  it('can be constructed with TLOS as output', () => {
    const trade = new Trade(
      new Route([pair_weth_0], token0, TLOS),
      CurrencyAmount.ether(JSBI.BigInt(100)),
      TradeType.EXACT_OUTPUT
    )
    expect(trade.inputAmount.currency).toEqual(token0)
    expect(trade.outputAmount.currency).toEqual(TLOS)
  })
  it('can be constructed with TLOS as output for exact input', () => {
    const trade = new Trade(
      new Route([pair_weth_0], token0, TLOS),
      new TokenAmount(token0, JSBI.BigInt(100)),
      TradeType.EXACT_INPUT
  ...
  describe('#bestTradeExactIn', () => {
    ...
    it('works for TLOS currency input', () => {
      const result = Trade.bestTradeExactIn(
    ...
    it('works for TLOS currency output', () => {
      ...
      expect(result[0].outputAmount.currency).toEqual(TLOS)
      expect(result[1].inputAmount.currency).toEqual(token3)
      expect(result[1].route.path).toEqual([token3, token1, token0, WETH[ChainId.MAINNET]])
      expect(result[1].outputAmount.currency).toEqual(TLOS)
    })
  })
