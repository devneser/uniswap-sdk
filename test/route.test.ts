import { Token, WETH, ChainId, Pair, TokenAmount, Route, TLOS } from '../src'
...
describe('Route', () => {
  ...
  it('supports ether input', () => {
    const route = new Route([pair_0_weth], TLOS)
    expect(route.pairs).toEqual([pair_0_weth])
    expect(route.input).toEqual(TLOS)
    expect(route.output).toEqual(token0)
  })

  it('supports ether output', () => {
    const route = new Route([pair_0_weth], token0, TLOS)
    expect(route.pairs).toEqual([pair_0_weth])
    expect(route.input).toEqual(token0)
    expect(route.output).toEqual(TLOS)
  })
})
